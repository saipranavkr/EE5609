...
\end{align}
A row of $\vec{R^{\prime}}$ can be all zeroes, or can start with zeroes from the left then has a one, and can have non-zero entries after one. 
$\vec{R^{\prime}}$ is in row-reduced echelon form. To perform column operations, elementary matrices should be multiplied on the right side in order to convert the $\vec{R^{\prime}}$ into column-echelon form
...
\end{align}
Hence, by using lemma it can be observed that ${\vec{Q}}^T$ is invertible and of the order $n\times n$. So, $\vec{Q}$ is also invertible. Converting $\vec{R}^T$ to row-reduced echelon is equivalent to converting $\vec{R}$ to column-reduced echelon. 
\begin{align} \label{eq:eq_4}
    \vec{R} = \vec{P}\vec{A}\vec{Q}
\end{align}
$\vec{R}$ in \eqref{eq:eq_4} is in both row and column reduced echelon form. Hence proved.
\end{document}