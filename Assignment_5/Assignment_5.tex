...
\centering
\resizebox{\columnwidth}{!}{\input{Assignment_5_triangle.tex}}
...
\end{align}
Area of triangle is,
\begin{align} \label{eq:eq_6a}
    Area(\triangle ABC) = \frac{1}{2}\norm{(\vec{A} - \vec{B}) \times (\vec{C} - \vec{B})}
\end{align}
Substituting \eqref{eq:eq_4}, \eqref{eq:eq_5} in \eqref{eq:eq_6a},
\begin{align} \label{eq:eq_6b}
...
\begin{figure}[!ht] \label{fig:triangle_abc}
\centering
\resizebox{\columnwidth}{!}{\input{Assignment_5_TwoTriangles.tex}}
...
\begin{align}
    ...
    \implies Area(\triangle DBC) =  \frac{1}{2}\norm{\vec{A}\times  \vec{C} } (\because \vec{A} \times \vec{A} = 0 ) \label{eq:eq_9}
\end{align}
It can be observed that \eqref{eq:eq_9} is same as \eqref{eq:eq_6b}
